...
//        }
  ...
  private SimplePooledConnection popConnection(String username, String password)
    ...
    while (conn == null) {
      synchronized (POOL_LOCK) {
    	 //a.如果还有空置连接则取空置连接链表第一个，将这个连接设置为可用，退出死循环
...
//          }
        //b.没有空置连接，则再去看活动连接链表大小是否超过了poolMaximumActiveConnections
        } else {
...
//            }
            ...
            if (longestCheckoutTime > poolMaximumCheckoutTime) {
	          ...
	          //a.如果连接AutoCommit=false则调用Connection.rollback，做这个操作的原因是因为
              //在这一步后面会将这个数据库连接封装成一个新的SimplePooledConnection给此时的
              //连接请求者使用，要保证目前使用者的事务完整性，就需要回滚...    
              if (!oldestActiveConnection.getRealConnection().getAutoCommit()) {
                oldestActiveConnection.getRealConnection().rollback();
              }
              //b.将这个连接封装成一个新的SimplePooledConnection，退出死循环
              conn = new SimplePooledConnection(oldestActiveConnection.getRealConnection(), this);
              
              //c.将这个对调用者暴露的连接SimplePooledConnection
              //标记为失效，调用者每次使用这个连接之前都会判断是否被设置失效
              //如果失效则会抛出运行时异常...
...
//              }
              ...
              // Must wait
            // 2. 休眠poolTimeToWait时间，休眠期间如果被中断，退出死循环，休眠结束后回到2.1.a继续循环...
              try {
                if (!countedWait) {
                  hadToWaitCount++;
                  countedWait = true;
